...
public class Auction {
   ...
   public List<Item> getSellerActiveAuctionItems() throws SQLException, ClassNotFoundException {
      String sql = "SELECT item.* FROM item JOIN auction ON item.itemId = auction.itemId WHERE auction.sellerID = ? AND status='active'";
      PreparedStatement statement = connection.prepareStatement(sql);
   ...
   public List<Item> getAllActiveAuctionItems() throws SQLException, ClassNotFoundException {
      String sql = "SELECT item.* FROM item JOIN auction ON item.itemId = auction.itemId WHERE auction.status='active' LIMIT 10";
      PreparedStatement statement = connection.prepareStatement(sql);
      ResultSet resultSet = statement.executeQuery();
      List<Item> itemList = new ArrayList<Item>();
   ...
   public String getAuctionId(String itemId) throws SQLException {
